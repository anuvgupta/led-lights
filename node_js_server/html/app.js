var app = {
            ...
            if (d != null) {
                ...
                switch (d.event) {
                    case 'auth':
                        ...
                        break;
                    case 'colorpalette':
                        app.block.child('controlpanel/colors/presets').on('clear', {
                            callback: function () {
                                for (var colorID in d.data) {
                                    if (d.data.hasOwnProperty(colorID)) {
                                        app.block.child('controlpanel/colors/presets').data({ newpreset: {
                                            r: d.data[colorID].r,
                                            g: d.data[colorID].g,
                                            b: d.data[colorID].b,
                                            id: colorID
                    ...
                    case 'newcolor':
                        ...
                        break;
                    case 'updatecolor':
                        app.block.child('controlpanel/colors/presets').data({ updatepreset: {
                            r: d.data.r,
                            g: d.data.g,
                            b: d.data.b,
                            id: d.data.id
                        ...
                        break;
                    case 'deletecolor':
                        app.block.child('controlpanel/colors/presets').data({ deletepreset: {
                            id: d.data.id
                        } });
                        break;
                    case 'patternlist':
                        for (var p in d.data) {
                            app.block.child('controlpanel/patterns').data({ newpattern: {
                                id: d.data[p].id,
                                name: d.data[p].name
                            } });
                        }
                    ...
                    case 'newpattern':
                        ...
                        break;
                    case 'loadpattern':
                        app.block.child('controlpanel/patterns').data({ patternname: {
                            id: d.data.id,
                            name: d.data.name
                        } });
                        app.block.child('controlpanel/patterns/area/editor').on('show');
                        app.block.child('controlpanel/patterns/area/editor/colors').data({
                            list: d.data.list
                        });
                    ...
                    case 'updatepattern':
                        ...
                        break;
                    case 'deletepattern':
                        app.block.child('controlpanel/patterns').data({ deletepattern: {
                            id: d.data.id
                        } });
                        if (app.currentPattern.id == d.data.id) {
    ...
    renamePattern: function (name) {
        if (app.socket.readyState == 1 && app.currentPattern.id.trim() != '') {
            app.socket.send(app.encodeMSG('renamepattern', { id: app.currentPattern.id, name: name }));
        }
    },
    addPatternColor: function () {
        if (app.socket.readyState == 1 && app.currentPattern.id.trim() != '') {
            app.socket.send(app.encodeMSG('addpatterncolor', { id: app.currentPattern.id }));
        }
    },
    updatePatternColor: function (colorID, colorData) {
        if (app.socket.readyState == 1 && app.currentPattern.id.trim() != '') {
            app.socket.send(app.encodeMSG('updatepatterncolor', { id: app.currentPattern.id, colorID: colorID, colorData: colorData }));
        }
    },
    movePatternColor: function (colorID, newPos) {
        if (app.socket.readyState == 1 && app.currentPattern.id.trim() != '') {
            app.socket.send(app.encodeMSG('movepatterncolor', { id: app.currentPattern.id, colorID: colorID, newPos: newPos }));
        }
    },
    deletePatternColor: function (colorID) {
        if (app.socket.readyState == 1 && app.currentPattern.id.trim() != '') {
            app.socket.send(app.encodeMSG('deletepatterncolor', { id: app.currentPattern.id, colorID: colorID }));
        }
    },
    ...
    sendBrightness: function () {
        if (app.socket.readyState == 1) {
            if (app.direct.brightnessAllowed) {
                ...
                app.socket.send(app.encodeMSG('setbrightness', { brightness: app.direct.brightness }));
            }
        }
    },
    sendSpeed: function () {
        if (app.socket.readyState == 1) {
            if (app.direct.speedAllowed) {
                ...
                app.socket.send(app.encodeMSG('setspeed', { speed: app.direct.speed }));
            }
        }
    },
    login: function (pass) {
    ...
    colorPickerUpdate: function (picker, update) {
        ...
        app.block.child('controlpanel/colors/picker').data({ color: {
    ...
    util: {
        ...
        rgbstring: function (r, g, b) {
            return app.util.lpad(String(parseInt(r)), 3, '0') + app.util.lpad(String(parseInt(g)), 3, '0') + app.util.lpad(String(parseInt(b)), 3, '0');
        }
    },
    img: {
